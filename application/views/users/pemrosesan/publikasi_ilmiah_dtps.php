    ...
    <div class="row">
      ...
      <div class="panel panel-flat">
        <div class="panel-heading">
          <h5 class="panel-title"><i class="icon-folder-download2"></i> Publikasi Ilmiah DTPS</h5>
            ...
            <ul class="icons-list">
              <li><a data-action="collapse"></a></li>
                ...
                if ($user->row()->level == 'admin' or $user->row()->level == 's_admin') { ?>
                    <br>
                    <a href="users/publikasi_ilmiah_dtps/t" class="btn btn-primary">+ <i class="icon-folder-download2"></i> Tambah Data</a>
                <?php
                } ?>
        </div>
        ...
        <table class="table datatable-basic" width="100%">
          ...
          <tbody>
              <?php
              $no = 1;
              foreach ($publikasi_ilmiah_dtps->result() as $baris) {
              ?>
                <tr>
                  <td><?php echo $no.'.'; ?></td>
                  ...
                  <td>
                    <a href="users/publikasi_ilmiah_dtps/d/<?php echo $baris->id_publikasi_ilmiah_dtps; ?>" class="btn btn-default btn-xs"><i class="icon-eye"></i></a>
                    
                    <?php
                    if ($user->row()->level == 's_admin' or $user->row()->level == 'admin' or $user->row()->level == 'timpjm' or $user->row()->level == 'pimpinan') { ?>
                    <a href="users/publikasi_ilmiah_dtps/e/<?php echo $baris->id_publikasi_ilmiah_dtps; ?>" class="btn btn-success btn-xs"><i class="icon-pencil7"></i></a>

                    <a href="users/publikasi_ilmiah_dtps/h/<?php echo $baris->id_publikasi_ilmiah_dtps; ?>" class="btn btn-danger btn-xs" onclick="return confirm('Apakah Anda Yakin Ingin Menghapusnya ?')"><i class="icon-trash"></i></a>
